...
Il permet:
 - de charger des mib
 - visualer les OID
 - d'intéroger un agent SNMP local ou distant pour peu que mibbrowser soit configurer correctement. C'est ce que vous allez faire afin de connecter depuis votre PC le conteneur Proxmox ou votre agent est installé. Rendez vous dans le menu -> Tools -> Option -> Onglet agent
